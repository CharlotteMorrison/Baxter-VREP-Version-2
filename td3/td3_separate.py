...
class TD3Separate(object):
    ...
    def __init__(self, mode):
        ...
        if mode == 'cooperative':        # cooperative (TD3)
        ...
        if mode == 'partial':           # partial (td3_shared_critic)
        ...
        if mode == 'independent':       # independent (td3separate)
        ...
        self.critic_1_target = Critic(self.state_dim_c1,  self.action_dim_c1).to(cons.DEVICE)
        ...
        if mode is 'independent':
            ...
            self.critic_2_target = Critic(self.state_dim_c2,  self.action_dim_c2).to(cons.DEVICE)
    ...
    def select_action(self, state, actor='combined', noise=cons.POLICY_NOISE):
        ...
        if noise != 0:
            ...
            noise = torch.clamp(torch.randn(self.action_dim_a1, dtype=torch.float32, device='cuda') * noise,
        ...
        for it in range(iterations):
            ...
            if it % cons.POLICY_FREQ == 0:  # update the actor policy less frequently (2)
                ...
                if self.mode == 'cooperative':
                    ...
                    actor_1_loss.backward(retain_graph=True)
